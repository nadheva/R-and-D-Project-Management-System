<x-app-layout>
        ...
        <div class="card-header pb-0 p-3">
          <div class="row">
            ...
            <div class="col-6 text-end">
                <a class="btn bg-gradient-dark mb-0" href="" data-bs-toggle="modal" data-bs-target="#tambahPerangkat"><i class="fas fa-plus"></i>&nbsp;&nbsp;Add User</a>
              </div>
          </div>
        </div>
        <div class="card">
            ...
            <table id="datatable-search" class="table align-items-center mb-0">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="align-middle text-center">
                    <span class="text-secondary text-xs font-weight-bold" maxlength="10" >{{ $i->email }}</span>
                  </td>
                  <td class="align-middle text-center">
                    <span class="text-secondary text-xs font-weight-bold" maxlength="10" >{{ $i->role}}</span>
                  </td>
                  <td>
                    <div class="align-middle text-center">
                      <form id="form-delete" action="{{route('user.destroy', $i->id)}}" method="POST" style="display: inline">
                        @csrf
                        @method("DELETE")
            ...
            <div class="modal-content">
                <form method="post" action="{{ route('user.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="tambahPerangkatLabel">Add User</h5>
                      ...
                      <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            @foreach(["admin" => "Admin", "user" => "User"] AS $role_value => $role_label)
    ...
    <div class="modal fade" id="editPerangkat-{{$i->id}}" tabindex="-1" role="dialog" aria-labelledby="editPerangkatLabel"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form method="post" action="{{ route('user-update', $i->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="recipient-name" class="col-form-label">Name:</label>
                            <input type="text" class="form-control" name="name" placeholder="*User name" value="{{$i->name}}" required>
                        </div>
                        <div class="form-group">
                            <label for="message-text" class="col-form-label">Image:</label>
                            <input type="file" class="form-control" name="image" placeholder="*User image" value="{{old("image", $i->image)}}" required>
                        </div>
                        <div class="form-group">
                            <label for="recipient-name" class="col-form-label">Email:</label>
                            <input type="email" class="form-control" name="email" placeholder="*User email" value="{{$i->email}}" required>
                        </div>
                        <div class="form-group">
                            <label for="recipient-name" class="col-form-label">Password:</label>
                            <input type="password" class="form-control" name="password" placeholder="*User password" value="{{$i->password}}" required>
                        </div>
                        <div class="form-group">
                            <label for="exampleFormControlSelect1" class="col-form-label">Role:</label>
                            <select class="form-control" name="role" id="exampleFormControlSelect1" required>
                            @foreach(["admin" => "Admin", "user" => "User"] AS $role_value => $role_label)
                            <option value="{{ $role_value }}" {{ old("role", $i->role) == $role_value ? "selected" : "" }}>{{ $role_label }}</option>
                            @endforeach
                            </select>
                          </div>
                    </div>
